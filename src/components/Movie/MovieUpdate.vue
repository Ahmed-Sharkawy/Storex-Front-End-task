                    ...
                    <div class="col-md-9 col-sm-12 col-xs-12">
                        <div class="blog-detail-ct">
                            <div class="comment-form">
                                ...
                                <form id="updateMovie" @submit.prevent="update">
                                    <div class="row">
                                        <div class="col-md-4">
                                            <input type="text" :value="movie.name" name="name" placeholder="Your name">
                                        </div>
                                        <div class="col-md-4">
                                            <div class="select-wrapper">
                                                <select name="category_id" class="select">
                                                    <option v-for="category in categorys" :key="category.id"
                                                    v-bind:value="category.id"  :selected="movie.category_id == category.id">{{ category.name }}</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <textarea name="description" v-model="movie.description" placeholder="Description"></textarea>
                                        </div>
                                    </div>
...
export default {
    ...
    methods: {
        ...
        update() {
            let createMovie = document.getElementById('updateMovie');
            let form = new FormData(createMovie);
            form.append('_method', 'PUT');

            axios.post(`movies/${this.$route.params.id}`, form, {
                headers: {
                    Authorization: 'Bearer ' + localStorage.getItem('token'),
                }
            }).then((r) => {
